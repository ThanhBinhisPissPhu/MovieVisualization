...
from reviews_spark import read_reviews_from_kafka, create_selection_df_reviews, insert_reviews
...
if __name__ == "__main__":
    ...
    if spark_conn and postgres_conn:
        ...
        # spark_df = read_ratings_from_kafka(spark_conn)
        # if spark_df:
        #     selection_df = create_selection_df_ratings(spark_df)
        ...
        # postgres_conn.close()
        spark_df = read_movies_from_kafka(spark_conn)
        if spark_df:
            selection_df = create_selection_df_movies(spark_df)
            if selection_df:
                movies = separate_movies_year_df(selection_df)
                ...
                streaming_movies_query = movies.writeStream\
